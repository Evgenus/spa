...
**randomize_urls** ─ параметр передается через шаблон `index_template` в загрузчик. Если `true` - загрузчик будет добавлять к адресам файлов и манифеста случайные байты для подавления кешрования. Значение по умолчанию `true`.
...
* `NoManifest()` - событие возникает если нет текущей версии, т.е. приложение небыло ещё загружено или было загружено с ошибками. Как правило в обработчике следует вызвать метод `checkUpdate` для первой загрузки. Параметров нет.
* `EvaluationStarted(manifest)` - событие возникает если попытка загрузки текущей версии приложения возможна. Параметры: `manifest` - текущий манифест, который будет загружен, в виде объекта.
* `ModuleEvaluated(module)` - событие возникает если модуль удачно был загружен. Параметры: `module` - загруженный модуль как объект.
* `EvaluationError(module, error)` - событие возникает если в процессе загрузки модуля возникла ошибка. Дальнейшая загрузка прекращается и событие `ApplicationReady` не возникнет. Параметры: `module` - модуль который не удалось загрузить (некоторые поля в нем могут отсутствовать); `error` - возникшая ошибка(некоторые распространенные ошибки явно типизированы).
* `ApplicationReady(manifest)` - событие возникает если все модули приложения были успешно загружены. Само приложение может в процессе своей загрузки установить обработчик этого события. Это событие можно считать стартом приложения. Параметры: `manifest` - текущий манифест, который был успешно загружен.

### checkUpdate

Метод инициирует проверку на существование более новой версии приложения. 

* `UpToDate()` - событие возникает если текущая версия приложения является также и самой новой. Параметры: `manifest` - текущий манифест, который был успешно загружен.
* `UpdateFound(event, manifest)` - событие возникает если найдена более новая версия приложения. Как правило в обработчике стоит вызвать метод `startUpdate` или запросить у пользователя подтверждение перед загрузкой файлов новой версии. Предполагается что в этот момент времени работает текущая версия приложения. Параметры: `event` - объект события переданный браузером как результат запроса; `manifest` - манифест новой версии. 
* `UpdateFailed(event, error)` - событие возникает если не удалось найти более новую версию по каким-то сторонним причинам или манифест новой версии не может быть загружен. Ошибка загрузки манифеста может произойти как в случае если манифест содержит неверные данные, так и в случае если текущая версия загрузчика устарела и не совместима с новым форматом манифеста. Параметры: `event` - объект события переданный браузером как результат запроса(из него можно получить ошибки связанные с скачиванием манифеста новой версии); `error` - ошибка возникшая в ходе анализа манифеста новой версии (может быть `null`).

### startUpdate

Метод инициирует скачивание новой версии приложения согласно полученному ранее манифесту.

* `ModuleBeginDownload(module)` - событие возникает когда послан запрос на скачивание модуля. Параметры: `module` - модуль как объект из манифеста новой версии, который должен быть скачан.
* `ModuleDownloadProgress(event, module)` - событие отображает процесс скачивания каждого модуля. Параметры: `event` - переданое браузером событие (из него можно получить количество байт которые уже скачаны); `module` - модуль который скачивается(из него можно получить общий размер модуля).
* `TotalDownloadProgress(progress)` - событие отображает общий прогресс скачивания модулей. `progress` - структура в которой есть поля: `loaded_count` - количество скачанных модулей, `total_count` - общее количество модулей, `loaded_size` - суммарное количество скачанных байт, `total_size` - суммарный общий размер модулей.
* `ModuleDownloadFailed(event, module)` - событие возникает когда скачивание модуля было прервано или завершилось ошибкой. Например когда не сошлась контрольная сумма. Параметры: `event` - переданое браузером событие, `module` - модуль который не удалось скачать.
* `ModuleDownloaded(module)` - событие возникает когда модуль был успешно скачан. Параметры: `event` - переданое браузером событие, `module` - модуль который удалось скачать.
* `UpdateCompleted(manifest)` - событие возникает когда все модули были успешно скачаны. Обработчик __должен вернуть__ `true`, если нужно перейти на новую версию, и `false` если нужно отклонить обновление. Обработчик также может уведомить пользователя о возможности загрузить новую версию или запланировать рестарт приложения. В таком случае новая версия будет загружена следующим вызовом метода `load`. Параметры: `manifest` - новый манифест, модули которого были успешно скачаны.
