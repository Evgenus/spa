...
* `EvaluationStarted(manifest)` - событие возникает если попытка загрузки текущей версии приложения возможна.
* `ModuleEvaluated(module)` - событие возникает если модуль удачно был загружен.
* `EvaluationError(error)` - событие возникает если в процессе загрузки модуля возникла ошибка. Дальнейшая загрузка прекращается и событие `ApplicationReady` не возникнет.
* `ApplicationReady(manifest)` - событие возникает если все модули приложения были успешно загружены. Само приложение может в процессе своей загрузки установить обработчик этого события. Это событие можно считать стартом приложения.

### checkUpdate

Метод инициирует проверку на существование более новой версии приложения. 

* `UpToDate(event)` - событие возникает если текущая версия приложения является также и самой новой.
* `UpdateFound(event, manifest)` - событие возникает если найдена более новая версия приложения. Как правило в обработчике стоит вызвать метод `startUpdate` или запросить у пользователя подтверждение перед загрузкой файлов новой версии. Предполагается что в этот момент времени работает текущая версия приложения.
...
* `ModuleBeginDownload(module)` - событие возникает когда послан запрос на скачивание модуля.
* `ModuleDownloadProgress(event, module)` - событие отображает процесс скачивания каждого модуля.
* `TotalDownloadProgress(progress)` - событие отображает общий прогресс скачивания модулей.
* `ModuleDownloadFailed(event, module)` - событие возникает когда скачивание модуля было прервано или завершилось ошибкой. Например когда не сошлась контрольная сумма.
* `ModuleDownloaded(event, module)` - событие возникает когда модуль был учпешно скачан.
