var Builder, CyclicDependenciesError, DB, ExternalDependencyError, Logger, Loop, ModuleTypeError, NoCopyingRuleError, UnresolvedDependencyError, clc, crypto, definition, detectiveAMD, detectiveCJS, ejs, encoders, eval_file, fs, get_config_content, globrules, hasBOM, hashers, load_json, load_yaml, packagejson, path, resolve, sandbox, vm, walker, yaml, _,
...
Builder = (function() {
  function Builder(options) {
    var name, pattern, template, type, value, _ref, _ref1, _ref10, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    ...
    this.hash_func = (_ref8 = options.hash_func) != null ? _ref8 : "md5";
    this.randomize_urls = (_ref9 = options.randomize_urls) != null ? _ref9 : true;
    this.coding_func = options.coding_func;
    this.cache = this._create_db(path.resolve(this.root, (_ref10 = options.cache_file) != null ? _ref10 : ".spacache"));
  }
      ...
      _results.push((function() {
        ...
        for (_i = 0, _len = deps.length; _i < _len; _i++) {
          ...
          if (this.grab && !this._by_path[resolved]) {
            submodule = {
              path: resolved,
              relative: this.walker.normalize(resolved)
            };
            submodule.type = this._get_type(submodule);
  ...
  Builder.prototype._create_manifest = function() {
    ...
    if (this.coding_func != null) {
      manifest.decoder_func = this.coding_func.name;
    }
    return manifest;
  };

  Builder.prototype._create_hosting_map = function() {
    var files, map, module, _i, _len, _ref;
    files = {};
    _ref = this._modules;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      module = _ref[_i];
  ...
  Builder.prototype.build = function() {
    var destination, hosting_map_content, hosting_map_data, manifest_content, manifest_data, module, output, source, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
    this._clear();
    ...
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      ...
      if (module.url == null) {
        this.logger.error("No hosting rules for `" + module.relative + "`");
      }
    }
    this._set_ids();
    this._link();
    ...
    if (this.manifest != null) {
      manifest_data = this._create_manifest();
      manifest_content = this._stringify_json(manifest_data);
      this._write_file(this.manifest, manifest_content);
    }
    if (this.hosting_map != null) {
      hosting_map_data = this._create_hosting_map();
      hosting_map_content = this._stringify_json(hosting_map_data);
      this._write_file(this.hosting_map, hosting_map_content);
    }
    if (this.index != null) {
      this._write_index();
    }
