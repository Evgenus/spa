var Builder, CryptoJS, CyclicDependenciesError, CypherFormatter, ExternalDependencyError, Logger, Loop, NoCopyingRuleError, ParameterError, PasswordParameterError, UnresolvedDependencyError, clc, cypher_algos, definition, detectiveAMD, detectiveCJS, ejs, encode_data, fs, get_config_content, globrules, hasBOM, hash_algos, load_json, load_yaml, packagejson, path, walker, yaml, _,
...
ParameterError = (function(_super) {
  ...
  function ParameterError() {
    return ParameterError.__super__.constructor.apply(this, arguments);
  }
...
PasswordParameterError = (function(_super) {
  __extends(PasswordParameterError, _super);

  function PasswordParameterError() {
    this.name = this.constructor.name;
    this.message = "`password` expected for encrypted files";
  }

  return PasswordParameterError;
...
cypher_algos = {
  identity: function(data, password) {
    return data;
  },
  aes: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.AES.encrypt(encoded_data, password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  },
  des: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.DES.encrypt(encoded_data, encoded_password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  },
  tripledes: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.TripleDES.encrypt(encoded_data, encoded_password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  },
  rabbit: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.Rabbit.encrypt(encoded_data, encoded_password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  },
  rc4: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.RC4.encrypt(encoded_data, encoded_password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  },
  rc4drop: function(data, password) {
    var encoded_data, encoded_password, encrypted;
    encoded_data = encode_data(data);
    encoded_password = encode_data(password);
    encrypted = CryptoJS.RC4Drop.encrypt(encoded_data, encoded_password, {
      format: CypherFormatter
    });
    return encrypted.toString();
  }
...
Builder = (function() {
  function Builder(options) {
    ...
    this.cypher_func = (_ref8 = options.cypher_func) != null ? _ref8 : "identity";
    if (!(this.cypher_func === "identity" || (options.password != null))) {
  ...
  Builder.prototype.calc_hash = function(content) {
    return hash_algos[this.hash_func](content);
  };

  Builder.prototype.calc_cypher = function(content) {
    return cypher_algos[this.cypher_func](content, this.password);
  };
  ...
  Builder.prototype._write_index = function() {
    ...
    namespace["decode_name"] = this.cypher_func;
  ...
  Builder.prototype.build = function() {
    ...
    for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
      ...
      if (!((destination != null) || this.cypher_func === "identity")) {
        throw new NoCopyingRuleError(module.relative);
      }
      output = this.calc_cypher(source);
      if (destination != null) {
