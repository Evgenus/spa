var Builder, CyclicDependenciesError, DB, ExternalDependencyError, Logger, Loop, ModuleTypeError, NoCopyingRuleError, UnresolvedDependencyError, clc, crypto, definition, detectiveAMD, detectiveCJS, ejs, eval_file, fs, get_config_content, globrules, hasBOM, load_json, load_yaml, packagejson, path, resolve, sandbox, vm, walker, yaml, _,
...
Builder = (function() {
  var _by_id, _by_path, _index_content, _manifest_content, _modules;
  ...
  function Builder(options) {
    var name, pattern, template, type, value, _ref, _ref1, _ref10, _ref11, _ref12, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    this._built_ins = ["loader"];
    this.options = options;
    if (_.isString(options.logger)) {
      this.logger = this._create_logger((_ref = options.logger) != null ? _ref : "SPA");
    } else {
      this.logger = options.logger;
    }
    this.root = path.resolve(process.cwd(), options.root) + "/";
    this.extensions = (_ref1 = options.extensions) != null ? _ref1 : [".js"];
    this.excludes = (_ref2 = options.excludes) != null ? _ref2 : [];
    this.paths = (_ref3 = options.paths) != null ? _ref3 : {};
    this.hosting = (function() {
      var _ref4, _ref5, _results;
      _ref5 = (_ref4 = options.hosting) != null ? _ref4 : {};
      _results = [];
      for (pattern in _ref5) {
        template = _ref5[pattern];
        _results.push({
    ...
    this.copying = (function() {
      var _ref4, _ref5, _results;
      _ref5 = (_ref4 = options.copying) != null ? _ref4 : {};
      _results = [];
      for (pattern in _ref5) {
        template = _ref5[pattern];
        _results.push({
    ...
    this.default_loader = (_ref4 = options.default_loader) != null ? _ref4 : "cjs";
    this.loaders = (function() {
      var _ref5, _ref6, _results;
      _ref6 = (_ref5 = options.loaders) != null ? _ref5 : {};
      _results = [];
      for (pattern in _ref6) {
        type = _ref6[pattern];
        _results.push({
          test: globrules.tester(pattern),
          type: type
    ...
    this.pretty = (_ref5 = options.pretty) != null ? _ref5 : false;
    this.grab = (_ref6 = options.grab) != null ? _ref6 : false;
    this.print_stats = (_ref7 = options.print_stats) != null ? _ref7 : true;
    this.print_roots = (_ref8 = options.print_roots) != null ? _ref8 : true;
    this.assets = {
      appcache_template: path.join(__dirname, "assets/appcache.tmpl"),
      index_template: path.join(__dirname, "assets/index.tmpl")
    };
    _ref9 = options.assets;
    for (name in _ref9) {
  ...
  Builder.prototype.calc_hash = function(content) {
    var hasher;
    hasher = this.constructor.hashers[this.hash_func];
    return hasher(content);
  };

  Builder.prototype.encode = function(content, module) {
    var encoder;
    encoder = this.constructor.encoders[this.coding_func.name];
  ...
  Builder.prototype._analyze = function() {
    var dep, deps, module, modules, resolved, source, submodule, _results;
    ...
    while (modules.length > 0) {
      ...
      _results.push((function() {
        ...
        for (_i = 0, _len = deps.length; _i < _len; _i++) {
          ...
          if (this.grab && !_by_path[resolved]) {
            submodule = {
              path: resolved,
            ...
            _results1.push(modules.push(submodule));
          } else {
  ...
  Builder.prototype._host = function() {
    var module, rule, _i, _j, _len, _len1, _ref, _results;
    _results = [];
    for (_i = 0, _len = _modules.length; _i < _len; _i++) {
      module = _modules[_i];
      _ref = this.hosting;
      ...
      if (module.url == null) {
        _results.push(this.logger.error("No hosting rules for `" + module.relative + "`"));
      } else {
  ...
  Builder.prototype._set_ids = function() {
    var ext, id, module, newroot, root, _i, _len;
    for (_i = 0, _len = _modules.length; _i < _len; _i++) {
      module = _modules[_i];
  ...
  Builder.prototype._link = function() {
    var dep, module, resolved, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = _modules.length; _i < _len; _i++) {
      module = _modules[_i];
      module.deps_ids = {};
      _results.push((function() {
        var _ref, _results1;
        _ref = module.deps_paths;
        _results1 = [];
        for (dep in _ref) {
          resolved = _ref[dep];
          if (_by_path[resolved] != null) {
            _results1.push(module.deps_ids[dep] = _by_path[resolved].id);
          } else {
  ...
  Builder.prototype._sort = function() {
    ...
    return _modules = (function() {
      var _j, _len1, _results;
      _results = [];
      for (_j = 0, _len1 = order.length; _j < _len1; _j++) {
        mpath = order[_j];
        _results.push(_by_path[mpath]);
      }
      return _results;
    })();
  };
  ...
  Builder.prototype._encode = function() {
    var destination, module, output, source, _i, _j, _len, _len1;
    if (this.coding_func != null) {
      ...
      for (_j = 0, _len1 = _modules.length; _j < _len1; _j++) {
        module = _modules[_j];
  ...
  Builder.prototype._create_manifest = function() {
    ...
    if (this.coding_func != null) {
      _manifest_content.decoder_func = this.coding_func.name;
    }
    return _manifest_content;
  };

  Builder.prototype._create_hosting_map = function() {
    var files, map, module, _i, _len;
    files = {};
    for (_i = 0, _len = _modules.length; _i < _len; _i++) {
      module = _modules[_i];
  ...
  Builder.prototype._create_index = function() {
    ...
    if (this.manifest != null) {
      ...
      if (url != null) {
        ...
        this.logger.warn("Manifest file hosted as `manifest.json` and will be accesible relatively");
      }
    }
  ...
  Builder.prototype._create_appcache = function() {
    ...
    if (this.index != null) {
      ...
      if (url != null) {
        ...
        assets[url] = this.calc_hash(_index_content);
      }
    }
  ...
  Builder.prototype._print_roots = function() {
    var all_deps, dep, dep_path, message, module, num, roots, _i, _j, _len, _len1, _ref, _ref1, _results;
    all_deps = [];
    for (_i = 0, _len = _modules.length; _i < _len; _i++) {
      module = _modules[_i];
      _ref = module.deps_ids;
      for (dep_path in _ref) {
        dep = _ref[dep_path];
        all_deps.push(dep);
      }
    }
    ...
    for (_j = 0, _len1 = _modules.length; _j < _len1; _j++) {
      module = _modules[_j];
      if (_ref1 = module.id, __indexOf.call(all_deps, _ref1) >= 0) {
        continue;
      }
      roots.push(module);
    }
  ...
  Builder.prototype._print_stats = function() {
    ...
    return this.logger.info("Total " + total + " bytes in " + _modules.length + " files");
  };
