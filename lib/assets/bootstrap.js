...
loader.onNoManifest = function(error) {
  ...
  loader.onModuleDownloadFailed = function(event, module, error) {
    var el, message, _ref, _ref1;
    log("onModuleDownloadFailed", event, module, error);
    message = (_ref = event != null ? (_ref1 = event.target) != null ? _ref1.statusText : void 0 : void 0) != null ? _ref : error;
    malfunction.report(("Error while downloading module `" + module.id + "` from url `" + module.url + "`: ") + message);
    el = $("#module-" + module.id);
    el.find(".state").addClass("hide");
    return el.find(".error").text(message).removeClass("hide");
  };
